...
- `ci.yaml`：`pull_request` 觸發，build 加測試，不登入 Docker Hub。
- `publish.yaml`：只由 push 到 `main` 觸發。build、測試、多架構 build、推送。`concurrency` 設定讓較新的執行取消較舊的。
- Renovate 追蹤 beancount、fava、beanprice、base image、uv image、GitHub Actions（釘選 commit SHA）。這比您說的「兩個基礎專案」多，目的是讓安全性更新也會重建 image。
- Renovate 頂層 `automerge: false`、`platformAutomerge: false`。只有 `patch`、`minor`、`digest`、`pin`、`pinDigest` 設為 automerge，由 Renovate 在 CI 通過後自行 merge。major 和其他類型都要您手動 merge。
...
|---|---|---|---|
| installed_packages_equal_lock | 列出 image 內 `/opt/venv` 的所有 distribution | 名稱和版本的集合等於 `uv export --locked --no-dev` 的集合；beancount 的 major 是 3；找不到 pytest | test |
| build_fails_on_lock_drift | 在暫存複本把 `pyproject.toml` 的 fava 改成 `1.30.15`，不改 `uv.lock`，執行 `docker build` | build 的 exit code 不是 0 | test |
...
| only_publish_workflow_pushes | 解析 `.github/workflows/*.yml` 和 `*.yaml` | `ci.yaml` 和 `publish.yaml` 都存在；只有 `publish.yaml` 引用 `secrets.DOCKERHUB_*`、login 或 push；它的 `on` 只有 push 到 `main`；它有 `concurrency` 且 `cancel-in-progress: true`；沒有 workflow 使用 `pull_request_target` | test |
| renovate_automerge_allowlist | 解析 `renovate.json` | 頂層 `automerge` 和 `platformAutomerge` 都是 false；設為 automerge 的 update type 集合恰為 `patch`、`minor`、`digest`、`pin`、`pinDigest`；`npx --yes --package renovate renovate-config-validator --strict` exit 0 | test |
...
- 2026-09-21 — v3 草稿（commit 2ce4f93）已撤回，沒有被核准。您決定這個 scope 之後不再走 spec 核准：after-implement squad 的發現直接修正，每項先有會失敗的測試。與 v2 文字不同的地方只有一處：`publish.yaml` 的 `cancel-in-progress` 改為 false（同一 group 依序執行），因為 true 會在推送中途取消執行。其餘修正見 `.scratch/image/squad/after-implement.md`。
